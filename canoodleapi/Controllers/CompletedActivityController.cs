...
public class CompletedActivityController : ControllerBase
{
    ...
    [HttpGet]
    [Route("GetHistoryJson/{completionId}")]
    public ApiResponseModel GetHistoryJson(int completionId)
    {
      
        try
        {
            string jsond = _completedActivityRepository.GetHistoryJson(completionId);
            _jsonData = string.Empty;
            if (jsond != null)
            {
                resultResponse.Data = jsond;
                resultResponse.IsError = false;
                _jsonData = JsonConvert.SerializeObject(jsond);
    ...
    [HttpGet]
    [Route("GetHistorySubActivityJson/{completedSubActivityID}")]
    public ApiResponseModel GetHistorySubActivityJson(int completedSubActivityID)
    {

        try
        {
            string jsond = _completedActivityRepository.GetHistorySubActivityJson(completedSubActivityID);
            _jsonData = string.Empty;
            if (jsond != null)
            {
                resultResponse.Data = jsond;
    ...
    [HttpGet]
    [Route("GetPercentage/{completionid}")]
    public ApiResponseModel GetPercentage(int completionid)
    {

        try
        {
            int jsond = _completedActivityRepository.GetPercentage(completionid);
            _jsonData = string.Empty;
            if (jsond != null)
            {
                resultResponse.Data = jsond;
