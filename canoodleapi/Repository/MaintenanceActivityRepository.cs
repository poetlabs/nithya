    ...
    public class MaintenanceActivityRepository : BaseRepository,IMaintenanceActivityRepository
    {
        ...
        public List<MaintenanceActivities> GetAllMaintanceactivityForHomescrren()
        {
            try
            {
                ...
                string sql = "SELECT ca.mcStatusID as CompletedStatusID,ma.* FROM MaintenanceActivities ma left join CompletedActivities ca on ma.activityId=ca.activityId and ca.mcStatusID not in (11,12) WHERE (mcintervalid = 3 AND  (DATEPART(day, '@CurrentDateTime') - DATEPART(day, (select top 1 updateddate from CompletedActivities where activityId=1 order by 1 desc))) = 1  ) " +
