    ...
    public class CompletedActivityRepository : BaseRepository, ICompletedActivityRepository
    {
        ...
        public List<CompletedActivities> GetAllOutstandingTask(int laborerid)
        {
            try
            {
                ...
                if (lbr?.IsAdmin == 1)
                {
                    sql = "select m.name as MachineName,Mc.mcommonname as StatusName,Ma.descriptions as Descriptions,ma.mcactivityTypeId,ma.IsPriority,ma.machineId,Ma.FilePath,C.* from CompletedActivities C inner join LaborerVisits L on C.VisitID=L.VisitID " +
