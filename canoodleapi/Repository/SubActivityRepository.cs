      ...
      //  public SubActivityRepository(DapperContext context) => _context = context;
        ...
        public List<SubActivities> GetSubActivitybyActivityid(int activityID,int CompletionId)
        {
            try
            {
                ...
                string sql = "select ca.CompletedSubActivityID as CompletedSubActivityID,mc.mcommonname as Status,ca.Comments as Comments,ca.ReadingValue as ReadingValue,ca.HistoryJson,sa.* from SubActivities sa inner join CompletedSubActivity ca on sa.SubActivityID=ca.SubActivityID " +
